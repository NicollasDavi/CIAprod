...
const Page = () => {
    ...
    const { matricula } = useRenderContext();
        ...
        axiosInstance.post('/doc', doc)
            .then(response => {
                if (response.data.status === 'error') {
                    setErrorMessage('Documento com esse título já existe.');
                    setTimeout(() => {
                        setErrorMessage('')
                    },3000)
                } else {
            ...
            .catch(error => {
                ...
                setErrorMessage('Ocorreu um erro ao salvar o documento.');
            })
            .finally(() => {
                setIsLoading(false);
            });
    ...
    const addNewType = async (type: number) => {
        ...
        if (image) {
            ...
            if (isPdfOrWord) {
                ...
                try {
                    const response = await axiosInstance.post('/arq', formData, {
                        headers: {
    ...
    const removeItem = (indexToRemove: number) => {
        setDoc(prevDoc => {
            ...
            return { ...prevDoc, types: updatedTypes };
        });
    };
