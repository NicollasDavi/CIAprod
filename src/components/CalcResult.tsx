  ...
  return (
    <div className='fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-blue-200 rounded-xl p-4 mt-20'>
      <h1 className='text-xl font-bold'>{`${curso} || ${unidade} em ${parcelamento}`}</h1>
        {mensalidadeManha && <h2>{`${turno} : ${parcelamento}x de R$ ${mensalidadeManha}`}</h2>}
        {mensalidadeTarde && <h2>{`${turno} : ${parcelamento}x de R$ ${mensalidadeTarde}`}</h2>}
        {mensalidadeNoite && <h2>{`${turno} : ${parcelamento}x de R$ ${mensalidadeNoite}`}</h2>}
        {mensalidadeOnline && <h2>{`${turno} : ${parcelamento}x de R$ ${mensalidadeOnline}`}</h2>}
      <br />
      {desconto > 0 && (
        <div>
          <h1>{`Valores com desconto para: ${aluno}`}</h1>
          {turnoManha && <h2>{`${turno} : ${parcelamento}x de R$ ${mensalidadeManhaDesconto}`}</h2>}
          {turnoTarde && <h2>{`${turno} : ${parcelamento}x de R$ ${mensalidadeTardeDesconto}`}</h2>}
          {turnoNoite && <h2>{`${turno} : ${parcelamento}x de R$ ${mensalidadeNoiteDesconto}`}</h2>}
          {turnoOnline && <h2>{`${turno} : ${parcelamento}x de R$ ${mensalidadeOnlineDesconto}`}</h2>}
        </div>
      )}
      <button
